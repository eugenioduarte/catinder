...
function CatSwipeComponent(
  { currentIndex, setCurrentIndex, catsList, handleLike }: CatSwipeProps,
  ref: React.Ref<CatSwipeRef>,
) {
  const theme = useTheme()
  const styles = catSwipeStyles(theme)
  const [cards, setCards] = useState(catsList)
  const topCardRef = React.useRef<{ swipe: (isLike: boolean) => void } | null>(
    null,
  )
  ...
  useImperativeHandle(ref, () => ({
    swipe: (isLike: boolean) => {
      topCardRef.current?.swipe(isLike)
    },
  }))
...
export default forwardRef(CatSwipeComponent)
...
const Card = forwardRef(
  (
    { cat, index, onSwipeEnd, handleLike, setCurrentIndex }: CardProps,
    ref: React.Ref<{ swipe: (isLike: boolean) => void }>,
  ) => {
    ...
    const finishSwipe = (isLike: boolean) => {
